...
var (
	instanceName = flag.String("name", "dendrite-p2p-pinecone-i2p", "the name of this P2P demo instance")
	instancePort = flag.Int("port", 8008, "the port that the client API will listen on")
	instancePeer = flag.String("peer", "", "the static Pinecone peers to connect to, comma separated-list")
	instanceDir  = flag.String("dir", ".", "the directory(create manually before use) to store the databases in (if --config not specified)")
...
func main() {
	...
	garlic, err := onramp.NewGarlic(*instanceName, onramp.SAM_ADDR, onramp.OPT_DEFAULTS) //maybe different privacy rules?
	if err != nil {
		panic(err)
	}
	defer garlic.Close()
	...
	go func() {
		...
		listener, err := garlic.Listen() //params dont matter in this world
