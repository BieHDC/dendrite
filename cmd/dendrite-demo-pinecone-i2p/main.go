...
func main() {
	...
	if instancePeer != nil && *instancePeer != "" {
		go func() { //connect in the background
...
func checkHTTP(conn net.Conn) (bool, net.Conn) {
	...
	/*numread,*/
	_, errpeek := connpeek.PeekOnce(thebytes)
...
func (pconn *peekerConn) PeekOnce(p []byte) (int, error) {
	buf := make([]byte, len(p))
	var numread int
	var err error
	if pconn.multiread != nil {
		//fmt.Println("peekerConn: re-read old stuff")
		numread, err = pconn.multiread.Read(buf)
	} else {
	...
	if numread != len(p) {
		//fmt.Println("peekerConn: we read too little")
		//if we read too little
		buf2 := make([]byte, numread)
		copy(buf2, buf)
		buf = buf2
	}
	copy(p, buf)
	pconn.multiread = io.MultiReader(bytes.NewReader(buf), pconn.Conn)
	pconn.lenbuf = len(buf)
...
func (pconn *peekerConn) Read(p []byte) (int, error) {
	if pconn.multiread != nil {
		...
		lenread, err := pconn.multiread.Read(p)
		if pconn.lenbuf < len(p) {
			//fmt.Println("peekerConn: multiread empty")
			pconn.multiread = nil //we have read all of it
			pconn.lenbuf = 0
		} else {
			pconn.lenbuf -= len(p) //reduce our restbuffer
